...
class AdminProductCreateBloc extends Bloc<AdminProductCreateEvent, AdminProductCreateState> {
  ...
  Future<void> _onImageUrlChanged(ImageUrlChanged event, Emitter<AdminProductCreateState> emit) async {
  ...
  Future<void> _onFormSubmit(FormSubmit event, Emitter<AdminProductCreateState> emit) async {
      ...
      Resource response = await productsUseCases.create.run(state.toProduct(), files.isNotEmpty ? files : null);
      emit(
        state.copyWith(
          response: response,
          formKey: formKey
